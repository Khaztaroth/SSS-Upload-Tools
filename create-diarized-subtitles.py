...
# If the file is shorter than the set max lenght then process directly 
    ...
    output_path = f'./audio_parser/segments/hdemucs_mmi/{audio_basename}_16_000/vocals.mp3'
...
model_a, metadata = whisperx.load_align_model(language_code=result['language'], device=MODEL_OPTIONS["device"], model_name="jonatasgrosman/wav2vec2-large-xlsr-53-english")
print(">> Aligning segments")
result_aligned = whisperx.align(result["segments"], model_a, metadata, source, MODEL_OPTIONS["device"], return_char_alignments=False, print_progress=True)
